...
class KalmanFilter:
    def __init__(self, q_scale=0.01, r_scale=0.5):
        ...
        # 过程噪声协方差 (Q)
        self.kf.processNoiseCov = np.eye(3, dtype=np.float32) * q_scale
    ...
    def predict(self, dt=None):
        if dt is not None:
            ...
            self.kf.transitionMatrix[0, 2] = 0.5 * dt**2
            self.kf.transitionMatrix[1, 2] = dt
        prediction = self.kf.predict()
        return prediction[0, 0] , prediction[1, 0] # 返回预测的位置和速度值，兼容原接口

    def update(self, measurement):
        measure = np.array([[np.float32(measurement)]])
        estimate = self.kf.correct(measure)
        return estimate[0, 0], estimate[1, 0] # 返回修正后的最优位置和速度值，兼容原接口

    def reset(self):
        # 状态置零，初始化
        self.kf.statePost = np.zeros((3, 1), np.float32)
        # 协方差重置为大值：告诉滤波器“我现在完全不确定位置”，强制它快速收敛到第一帧观测值
        self.kf.errorCovPost = np.eye(3, dtype=np.float32) * 1000.0
